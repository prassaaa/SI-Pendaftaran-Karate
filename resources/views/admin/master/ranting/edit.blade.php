    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-green-600 to-emerald-600 px-8 py-6">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-white">Edit Ranting</h1>
                    <p class="text-green-100">Perbarui informasi ranting/cabang karate INKAI</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100">
                ...
                <p class="text-gray-500 mt-1">Perbarui informasi ranting di bawah ini</p>
            </div>

            <div class="p-8">
                <form action="{{ route('admin.master.ranting.update', $ranting->id) }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text"
                               id="nama_ranting"
                               name="nama_ranting"
                               value="{{ old('nama_ranting', $ranting->nama_ranting) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="provinsi"
                                ...
                                <option value="Jawa Timur" {{ old('provinsi', $ranting->provinsi) == 'Jawa Timur' ? 'selected' : '' }}>Jawa Timur</option>
                                <option value="Jawa Tengah" {{ old('provinsi', $ranting->provinsi) == 'Jawa Tengah' ? 'selected' : '' }}>Jawa Tengah</option>
                                <option value="Jawa Barat" {{ old('provinsi', $ranting->provinsi) == 'Jawa Barat' ? 'selected' : '' }}>Jawa Barat</option>
                                <option value="DKI Jakarta" {{ old('provinsi', $ranting->provinsi) == 'DKI Jakarta' ? 'selected' : '' }}>DKI Jakarta</option>
                                <option value="Banten" {{ old('provinsi', $ranting->provinsi) == 'Banten' ? 'selected' : '' }}>Banten</option>
                                <option value="DI Yogyakarta" {{ old('provinsi', $ranting->provinsi) == 'DI Yogyakarta' ? 'selected' : '' }}>DI Yogyakarta</option>
                                <option value="Bali" {{ old('provinsi', $ranting->provinsi) == 'Bali' ? 'selected' : '' }}>Bali</option>
                                <option value="Sumatera Utara" {{ old('provinsi', $ranting->provinsi) == 'Sumatera Utara' ? 'selected' : '' }}>Sumatera Utara</option>
                                <option value="Sumatera Barat" {{ old('provinsi', $ranting->provinsi) == 'Sumatera Barat' ? 'selected' : '' }}>Sumatera Barat</option>
                                <option value="Sumatera Selatan" {{ old('provinsi', $ranting->provinsi) == 'Sumatera Selatan' ? 'selected' : '' }}>Sumatera Selatan</option>
                                <option value="Kalimantan Timur" {{ old('provinsi', $ranting->provinsi) == 'Kalimantan Timur' ? 'selected' : '' }}>Kalimantan Timur</option>
                                <option value="Kalimantan Selatan" {{ old('provinsi', $ranting->provinsi) == 'Kalimantan Selatan' ? 'selected' : '' }}>Kalimantan Selatan</option>
                                <option value="Sulawesi Selatan" {{ old('provinsi', $ranting->provinsi) == 'Sulawesi Selatan' ? 'selected' : '' }}>Sulawesi Selatan</option>
                                <option value="Sulawesi Utara" {{ old('provinsi', $ranting->provinsi) == 'Sulawesi Utara' ? 'selected' : '' }}>Sulawesi Utara</option>
                            </select>
                            @error('provinsi')
                                <p class="text-red-500 text-sm flex items-center space-x-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="alamat"
                                  ...
                                  required>{{ old('alamat', $ranting->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-500 text-sm flex items-center space-x-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="tel"
                               id="kontak"
                               name="kontak"
                               value="{{ old('kontak', $ranting->kontak) }}"
