...
<head>
    ...
    <title>Laporan Clustering Berat Badan Peserta</title>
    <style>
        body {
...
<body>
    <!-- Header -->
    <div class="header">
        <h1>LAPORAN CLUSTERING BERAT BADAN PESERTA</h1>
    ...
    <div class="statistics">
        ...
        <div class="stats-grid">
            ...
            <div class="stats-row">
                <div class="stats-cell stats-label">Rata-rata Berat:</div>
                <div class="stats-cell">{{ $statistics['avg_weight'] }} kg</div>
            </div>
            <div class="stats-row">
                <div class="stats-cell stats-label">Rentang Berat:</div>
                <div class="stats-cell">{{ $statistics['min_weight'] }} - {{ $statistics['max_weight'] }} kg</div>
            </div>
            <div class="stats-row">
    ...
    <div class="cluster-summary">
        <h3>Ringkasan Cluster Berat Badan</h3>
        <table class="cluster-table">
            <thead>
                <tr>
                    ...
                    <th>Berat Teringan</th>
                    <th>Berat Terberat</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $cluster['min'] }}-{{ $cluster['max'] > 100 ? '∞' : $cluster['max'] }} kg</td>
                    <td>{{ $cluster['count'] }} orang</td>
                    <td>{{ $cluster['percentage'] }}%</td>
                    <td>{{ $cluster['avg_weight'] }} kg</td>
                    <td>{{ $cluster['min_weight'] }} kg</td>
                    <td>{{ $cluster['max_weight'] }} kg</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    @foreach($clusteringData['clusters'] as $name => $cluster)
        ...
        <div class="cluster-detail {{ !$loop->first ? 'page-break' : '' }}">
            ...
            <div class="cluster-info">
                <p><strong>Rentang Berat:</strong> {{ $cluster['min'] }}-{{ $cluster['max'] > 100 ? '∞' : $cluster['max'] }} kg</p>
                <p><strong>Jumlah Peserta:</strong> {{ $cluster['count'] }} orang ({{ $cluster['percentage'] }}%)</p>
                <p><strong>Rata-rata Berat:</strong> {{ $cluster['avg_weight'] }} kg</p>
                <p><strong>Rentang Berat Aktual:</strong> {{ $cluster['min_weight'] }}-{{ $cluster['max_weight'] }} kg</p>
            </div>
